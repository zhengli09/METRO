  ...
  )
{
  ...
  arma::vec onesP = arma::ones<arma::vec>(p); // p x 1 vector of ones
  ...
  while(iter < 2 || (iter < maxIter && std::abs(logliks(iter - 1) - logliks(iter - 2)) >= tol))
  {
    ...
    mubeta = Sigmabeta * 
      (
        arma::kron((nz - 1.0) % (1.0 / sigma2m), onesP) % betaeQTL + 
        (n - 1.0) / sigma2 * arma::kron(alpha, betaGWAS) -
        (n - 1.0) / sigma2 * gamma * arma::kron(alpha, D * onesP)
      );
    ...
    logliks.push_back(
      ...
      0.5 * (n - 1.0) * (1.0 + gamma * gamma * arma::accu(D) - 
        2.0 * gamma * arma::sum(betaGWAS)) / sigma2 +
    ...
    // 2. update gamma
    gamma = (arma::sum(betaGWAS) - 
      arma::as_scalar(mubeta.t() * arma::kron(alpha, D) * onesP)
      ) / arma::accu(D);
    ...
    sigma2 = (1.0 + 
      ...
      gamma * gamma * arma::accu(D) - 
      2.0 * gamma * arma::sum(betaGWAS) +
      2.0 * gamma * arma::as_scalar(mubeta.t() * arma::kron(alpha, D * onesP))
      );
